...
namespace lpzrobots {
   ...
   */
  OdeRobot::OdeRobot(const OdeHandle& odeHandle, const VsgHandle& vsgHandle,
                     const std::string& name,const std::string& revision)
    : AbstractRobot(name, revision),
      initialPose(vsg::dmat4(1.0)),
      initialRelativePose(vsg::dmat4(1.0)),
      odeHandle(odeHandle), vsgHandle(vsgHandle),
  ...
  void OdeRobot::placeIntern(const vsg::dmat4& pose){
  ...
  */
  ...
  void OdeRobot::moveToPose(Pose pose, int primitiveID){
    ...
    Pose refInvertPose = Pose(inverse(static_cast<vsg::dmat4>(ref->getPose())));
